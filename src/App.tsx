...
import { GoogleMap, useJsApiLoader, Polyline, Marker as GoogleMarker, DirectionsService, DirectionsRenderer } from '@react-google-maps/api';
...
function getNextDeliveryDay(scheduleStr: string): string | null {
  ...
  const now = new Date();
  const today = now.getDay(); // 0 is Sunday, 1 is Monday, ..., 6 is Saturday
  ...
  // Find the first day in the schedule that is after today
  const nextDayIndex = daysInSchedule.find(day => day > today);
  
  if (nextDayIndex !== undefined) {
    if (nextDayIndex === today + 1) {
      return 'Tomorrow';
    }
    return JS_DAYS[nextDayIndex];
  }
  
  // If no day found after today, it's the first day of next week
...
export default function App() {
  ...
  const toggleDay = (day: string) => {
    ...
    // Sort days to keep schedule string consistent
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        fetchAllTerritories();
        if (query) {
          setResults(results.filter(t => t.id !== deleteConfirmId));
        }
    ...
    const searchTerritories = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Smart Search Logic: If no direct matches, try Gemini
          if (data.length === 0) {
            try {
              ...
              if (allRes.ok) {
                ...
                const prompt = `
                  ...
                  1. The starting point (terminal) for all routes is 18100 E 40th Ave #20, Aurora, CO 80011. Do not mention this address in the output, but use it to calculate "on the way" logic.
                  2. STRICT STATE LIMIT: Only consider locations and businesses within Colorado (CO) and Wyoming (WY). Completely ignore any locations of a business outside of these two states.
                  3. Use "on the way" logic. For example, Glenwood Springs is on the way to Grand Junction from Aurora. If a user searches for a city that is geographically on the route to one of our major stops, match it to that territory.
                  4. If the query is a specific city, check if it's near or on the path to any of the cities in our list.
                  5. SPECIFIC ROUTING RULE: Any stop between Idaho Springs and Estes Park (including Nederland, Black Hawk, Central City, etc.) must be matched to the "Clear Creek" (Idaho Springs) territory, NOT the "Flatirons" (Boulder) territory.
                  6. CHAIN RESTAURANTS & BUSINESSES: If the user searches for a business name (e.g., "Federales", "Beau Jo's"):
                     - FIRST, check how many locations exist in CO and WY.
                     - If there is ONLY ONE location in CO/WY (e.g., "Federales" only has one CO location in RiNo/Denver), match it to the correct territory for that specific location (e.g., Downtown).
                     - ONLY if there are MULTIPLE locations in CO/WY, return 0 for matchId and provide a message asking them to specify the city or zip code.
                  ...
                  Return a JSON object. If there is a match, return matchId and a brief reason. If it's a chain with multiple CO/WY locations and no city specified, return matchId: 0 and a message. If no match is found, return matchId: 0.
                `;
                ...
                const engineRes = await engine.models.generateContent({
                  ...
                  config: {
                    ...
                    responseSchema: {
                      type: Type.OBJECT,
                      properties: {
                        matchId: { type: Type.INTEGER, description: 'The ID of the matching territory, or 0' },
                        reason: { type: Type.STRING, description: 'Briefly explain why this is a match' },
                        message: { type: Type.STRING, description: 'Message to show the user if more info is needed' }
                      }
                    }
                  }
                });
                ...
                if (matchData.matchId > 0) {
                  ...
                  if (match) {
                    data = [{ ...match, matchReason: matchData.reason }];
                  }
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-[#f8f9fa] font-sans text-black flex flex-col">
      ...
      <div className="absolute top-6 right-6 z-10 flex items-center gap-3">
        <div className="flex bg-white/80 backdrop-blur-md rounded-xl border border-black/5 shadow-sm overflow-hidden">
          <button 
            ...
            className={cn("px-4 py-2 text-[10px] font-bold tracking-[0.05em] transition-all", viewMode === 'search' ? "bg-black text-white" : "text-black/40 hover:bg-black/5")}
          >
            Search
          </button>
          <button 
            onClick={() => setViewMode('list')}
            className={cn("px-4 py-2 text-[10px] font-bold tracking-[0.05em] transition-all border-l border-black/5", viewMode === 'list' ? "bg-black text-white" : "text-black/40 hover:bg-black/5")}
          >
            List
          </button>
          <button 
            onClick={() => setViewMode('map')}
            className={cn("px-4 py-2 text-[10px] font-bold tracking-[0.05em] transition-all border-l border-black/5", viewMode === 'map' ? "bg-black text-white" : "text-black/40 hover:bg-black/5")}
          >
            Map
          </button>
        </div>
        {isAuthenticated ? (
          <>
            ...
            <button 
              ...
              className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-black/5 text-black/20 transition-all border border-black/5 bg-white"
            >
              ✕
            </button>
          </>
          ...
          <button 
            ...
            className="text-[10px] font-bold text-black/40 hover:text-black transition-all tracking-[0.05em] px-4 py-2 rounded-xl border border-black/5 hover:bg-white"
          >
            Admin
          </button>
        )}
      ...
      <header className={cn(
        ...
        (hasSearched && viewMode === 'search') || viewMode !== 'search' ? "pt-12 pb-6" : "pt-24 pb-12"
      )}>
        <div className="max-w-5xl mx-auto px-4 flex flex-col items-center animate-in">
          <img 
            ...
            alt="Logo" 
            className={cn(
              "w-auto transition-all duration-700 grayscale contrast-125 opacity-80",
              (hasSearched && viewMode === 'search') || viewMode !== 'search' ? "h-10 mb-2" : "h-20 mb-6"
            )}
            referrerPolicy="no-referrer"
          />
          <div className="text-center">
            <h1 className={cn(
              "font-bold text-black tracking-tight transition-all duration-700",
              (hasSearched && viewMode === 'search') || viewMode !== 'search' ? "text-lg" : "text-3xl"
            )}>
              Find Your Delivery Route
            </h1>
            <p className={cn(
              "text-black/40 font-medium transition-all duration-700 mt-2",
              (hasSearched && viewMode === 'search') || viewMode !== 'search' ? "text-xs opacity-0 h-0 overflow-hidden" : "text-sm opacity-100"
            )}>
              Enter your city, zip code, or customer name to find your delivery schedule.
            </p>
          </div>
        </div>
      </header>
          ...
          {viewMode === 'search' ? (
            <div className="flex-1 flex flex-col min-h-0 space-y-8 animate-in">
              ...
              <div className="relative shrink-0">
                <Search 
                  size={20} 
                  className="absolute left-6 top-1/2 -translate-y-1/2 text-black/20 pointer-events-none z-10" 
                />
                <input
                  type="text"
                  className={cn(
                    "input-field !pl-16 py-6 text-lg shadow-[0_10px_40px_-15px_rgba(0,0,0,0.1)] transition-all duration-500 bg-white",
                    isSearching && "border-black/20 ring-4 ring-black/5"
                  )}
                ...
                />
                {isSearching && (
                  <div className="absolute inset-y-0 right-0 pr-6 flex items-center text-black/40">
                    <Loader2 size={18} className="animate-spin" />
                  </div>
                )}
                {/* Search Progress Bar */}
                {isSearching && (
                  <div className="absolute -bottom-4 left-0 w-full h-[2px] bg-black/5 overflow-hidden rounded-full">
                    <div className="h-full w-1/2 bg-black animate-progress rounded-full" />
                  </div>
                )}
              </div>
                  ...
                  <div className="space-y-4 pb-8">
                    {results.map((territory) => {
                      ...
                      return (
                        <div 
                          key={territory.id} 
                          className="card group bg-white flex flex-col gap-5"
                        >
                          <div className="min-w-0">
                            <div className="flex items-center gap-3 mb-2">
                              ...
                              {territory.matchReason && (
                                <span className="text-[8px] font-bold text-black tracking-wider bg-black/5 px-2 py-0.5 rounded-full">
                                  Verified Match
                                </span>
                              )}
                            </div>
                          ...
                          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-black/5">
                            ...
                            <button 
                              onClick={() => setSelectedRouteForMap(territory)}
                              className="flex items-center gap-2 text-[10px] font-bold tracking-wider text-black/40 hover:text-black transition-all group/btn"
                            >
                ...
                <ComposableMap projection="geoMercator" projectionConfig={{ scale: 4500, center: [-105.5, 40.5] }} className="w-full h-full">
                      ...
                      geographies.map(geo => {
                        ...
                        return (
                          <Geography
                            key={geo.rsmKey}
                            geography={geo}
                            fill={isTargetState ? "#f5f5f5" : "#ffffff"}
                            stroke="#cccccc"
                            strokeWidth={isTargetState ? 1.5 : 0.5}
                          />
                        );
                      })
                    }
                      ...
                      <g key={t.id}>
                        ...
                        <Marker coordinates={coords}>
                          ...
                          <text
                            textAnchor="middle"
                            y={-12}
                            style={{ fontFamily: "Inter", fill: "#000", fontSize: "10px", fontWeight: "700", letterSpacing: "-0.02em" }}
                          >
                            {t.city.split(',')[0]}
                          </text>
                        </Marker>
                  ...
                  <Marker coordinates={AURORA_COORDS}>
                    ...
                    <text
                      textAnchor="middle"
                      y={18}
                      style={{ fontFamily: "Inter", fill: "#000", fontSize: "10px", fontWeight: "900", letterSpacing: "0.05em" }}
                    >
                      Terminal
                    </text>
                  </Marker>
            ...
            <div className="flex-1 flex flex-col min-h-0 space-y-6 animate-in">
              <div className="flex items-center justify-between px-2">
                ...
                <span className="text-[10px] font-bold text-black/20 tracking-wider">
                  {allTerritories.length} Total
                </span>
              </div>
      ...
      {/* Footer */}
      <footer className="py-6 text-center text-black/30 text-xs font-medium">
            ...
            <div className="p-8">
              <div className="flex items-center justify-between mb-8">
                ...
                <button 
                  ...
                  className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-black/5 text-black/20 transition-all"
                >
                  ✕
                </button>
              </div>
              ...
              <form onSubmit={handlePasscodeSubmit} className="space-y-6">
                <div className="space-y-2">
                  ...
                  />
                  {passcodeError && (
                    <p className="text-[10px] text-red-500 font-bold tracking-wider text-center mt-2">
                      Incorrect Passcode
                    </p>
                  )}
                </div>
                ...
                <button
                  type="submit"
                  className="w-full py-4 bg-black text-white rounded-2xl font-bold text-xs tracking-[0.05em] hover:bg-black/90 transition-all shadow-xl shadow-black/10"
                >
                  Verify Access
                </button>
              </form>
                ...
                <div className="flex items-center gap-2">
                  {editingTerritory && (
                    <button 
                      onClick={() => {
                        setIsAdminOpen(false);
                        handleDeleteTerritory(editingTerritory.id);
                      }}
                      className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-red-50 text-red-500 transition-all"
                      title="Delete Route"
                    >
                      <Trash2 size={18} />
                    </button>
              ...
              <form onSubmit={handleAddTerritory} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    placeholder="e.g. I-70 West Route"
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {DAYS_OF_WEEK.map(day => (
                      <button
                        ...
                        className={cn(
                          "px-2 py-2.5 rounded-xl text-[10px] font-bold tracking-wider transition-all border",
                          selectedDays.includes(day)
                            ? "bg-black border-black text-white shadow-lg shadow-black/10"
                            : "bg-white border-black/5 text-black/30 hover:border-black/20"
                        )}
                      >
                        {day.slice(0, 3)}
                      </button>
                  ...
                  <input
                    required
                    type="hidden"
                    value={newTerritory.schedule}
                  />
                  {newTerritory.schedule && (
                    <p className="text-[10px] text-black font-bold tracking-wider mt-2 px-1 opacity-40">
                      {newTerritory.schedule}
                    </p>
                  )}
                </div>
                ...
                <button
                  disabled={isAdding}
                  type="submit"
                  className="w-full py-4 bg-black text-white rounded-2xl font-bold text-xs tracking-[0.05em] hover:bg-black/90 transition-all disabled:opacity-50 shadow-xl shadow-black/10"
                >
            ...
            <div className="p-8 text-center">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className="py-4 bg-black/5 text-black rounded-2xl font-bold text-xs tracking-[0.05em] hover:bg-black/10 transition-all"
                >
                  Cancel
                </button>
                <button
                  onClick={confirmDelete}
                  className="py-4 bg-red-500 text-white rounded-2xl font-bold text-xs tracking-[0.05em] hover:bg-red-600 transition-all shadow-xl shadow-red-500/20"
                >
                  Delete
                </button>
              </div>
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-6xl h-[85vh] overflow-hidden border border-black/5 flex flex-col">
            <div className="p-8 border-b border-black/5 flex items-center justify-between shrink-0">
              ...
              <button 
                ...
                className="w-12 h-12 flex items-center justify-center rounded-full bg-black/5 hover:bg-black hover:text-white transition-all"
              >
                ✕
              </button>
            </div>
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-black text-white p-6 rounded-3xl shadow-xl shadow-black/10">
                    ...
                    <p className="text-[10px] text-white/60 leading-relaxed italic">
                      Routes are subject to weather conditions and terminal capacity.
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center gap-3 text-xs font-medium text-black/60">
                      ...
                      <span>Origin: Aurora Terminal</span>
                    </div>
                    <div className="flex items-center gap-3 text-xs font-medium text-black/60">
                      <div className="w-2 h-2 rounded-full bg-black/20" />
                      <span>Destination: {selectedRouteForMap.city}</span>
                    </div>
                  </div>
                </div>
              </div>
                ...
                {!googleMapsApiKey ? (
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-12 text-center">
                    <div className="w-16 h-16 rounded-full bg-black/5 flex items-center justify-center mb-6">
                      <AlertCircle className="text-black/20" size={32} />
                    </div>
                    ...
                    <p className="text-sm text-black/40 max-w-xs leading-relaxed">
                      Please add your <code className="bg-black/5 px-1 rounded text-black/60">VITE_GOOGLE_MAPS_API_KEY</code> to the Secrets panel.
                    </p>
                  </div>
                ) : loadError ? (
                  <div className="absolute inset-0 flex flex-col items-center justify-center p-12 text-center">
                    ...
                    <div className="text-sm text-black/40 max-w-sm space-y-4 leading-relaxed">
                      <p>Your API key is blocked from loading the map. Please check your <span className="font-bold text-black/60">Google Cloud Console</span>.</p>
                    </div>
                  </div>
                  ) : isLoaded ? (
                        ...
                        <div className="absolute inset-0 z-10 bg-white/90 backdrop-blur-sm flex items-center justify-center p-8 text-center">
                          <div className="max-w-md">
                            ...
                            <div className="text-sm text-black/40 space-y-4 leading-relaxed">
                              <p>The map is loaded, but the <span className="font-bold text-black/60">Directions API</span> is not enabled in your Google Cloud Console.</p>
                              <div className="text-left bg-black/5 p-4 rounded-2xl text-[11px] space-y-2">
                                <p>1. Go to <span className="font-bold text-black/60">APIs & Services &gt; Library</span></p>
                                <p>2. Search for <span className="font-bold text-black/60">"Directions API"</span></p>
                                <p>3. Click <span className="font-bold text-black/60">Enable</span></p>
                              </div>
                              <p className="text-[10px] italic">Note: It may take a few minutes to activate after enabling.</p>
                            </div>
                          </div>
                        </div>
                      )}
                          ...
                          {(() => {
                            ...
                            return (
                              <>
                                ...
                                <GoogleMarker 
                                  position={AURORA_COORDS_OBJ} 
                                  icon={{
                                    url: BLACK_DOT_ICON_URL,
                                ...
                                <GoogleMarker 
                                  ...
                                  title={selectedRouteForMap.city}
                                />
                              </>
                            );
                          })()}
